...
def createPlot(simTag, objTag, x, y, objFolder=os.path.join('data','obj'),  z=None, plotType='scatter', title='Plot', labels={'x':'x', 'y':'y', 'z':'z'}, makeNote=True, **kwargs):
    ...
    if plotType == 'scatter':
        if z is not None:
            fig = px.scatter_3d(df, x='x', y='y', z='z', title=title, labels=labels, **kwargs)
        else:
